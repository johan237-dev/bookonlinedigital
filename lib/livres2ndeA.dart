...
class Livres2ndeA extends StatelessWidget {
  Livres2ndeA({super.key});
  final CollectionReference livres2ndeA = FirebaseFirestore.instance.collection("Livres2ndeA");

  @override
  Widget build(BuildContext context) {
    return  Scaffold(
        appBar: AppBar(
          title: const Text("Livres de la classe de 2ndeA4",
            style: TextStyle(
                fontSize: 24,
        ...
        body: Padding(
          ...
          child:StreamBuilder(
              stream: FirebaseFirestore.instance.collection("Livres2ndeA").snapshots(),
              builder:(BuildContext context, AsyncSnapshot<QuerySnapshot>snapshot){
                if(snapshot.connectionState == ConnectionState.waiting){
                  return const CircularProgressIndicator();
                }
                if(!snapshot.hasData){
                  return const Text("Aucun livres trouvé");
                }
